...
(function(){
	...
	*/
	...
	var AutoComplete = {
		init: function(AutoCompleteData) {
			var AutoCompleteArray = [];

			for (var i = 0; i < AutoCompleteData.length; i++) {
				var keyWords = AutoCompleteData[i].keyWord.split("|");
				var url = AutoCompleteData[i].targetURL;
			...
			Extension.Element.get("input.search-field__input", function(selector, elem) {
				if (window.autoComplete) {
					new autoComplete({
						selector: 'input.search-field__input',
						minChars: 1,
						source: function(term, suggest) {
							term = term.toLowerCase();
							var choices = AutoCompleteArray;
							var matches = [];
							for (i = 0; i < choices.length; i++)
								if (choices[i].value.toLowerCase().indexOf(term) > -1)
						...
						renderItem: function(item, search) {
							search = search.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
							var re = new RegExp("(" + search.split(' ').join('|') + ")","gi");
							return '<div class="autocomplete-suggestion" data-url="' + item.url + '" data-val="' + search + '">' + item.value.replace(re, "<b>$1</b>") + '</div>';
						},
						onSelect: function(e, term, item) {
							try {
								document.querySelector('input.search-field__input').value = item.innerHTML.replace(/<b>|<\/b>/g, "");
							} catch (err) {}
							e.preventDefault();
	...
	var transfer = {
		objects: [
			{name: "AutoComplete", fn: AutoComplete}
		],
		dependencies: [],
		bootup: function(){
			Extension.Version.load('extlogs', false, function() {
				var str = ""; if (preview) str = "Preview";
				console.log("Bootup AutoComplete " + str);
			});
			//Initiate AutoComplete
			if (Extension.Common.checkMobile()) {
				Extension.Source.load('AutoComplete', Extension.Version.forcePreview || Extension.Version.preview, function(data) {

					Extension.AutoComplete.init(data.autocompletedata);
				});
			}
		}
	}
